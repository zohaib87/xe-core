...
class acf_field_extended_color_picker extends acf_field {
	...
	*/
	...
	function __construct() {
		...
		$this->label = __("RGBA Color Picker",'xe-core');
	...
	function input_admin_enqueue_scripts() {
		...
		if( !isset($wp_scripts->registered['iris']) ) {
			...
			wp_localize_script('wp-color-picker', 'wpColorPickerL10n', array(
				'clear'			=> __('Clear', 'xe-core' ),
				'defaultString'	=> __('Default', 'xe-core' ),
				'pick'			=> __('Select Color', 'xe-core' ),
				'current'		=> __('Current Color', 'xe-core' )
			));
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		// color palette
		acf_render_field_setting( $field, array(
			'label'			=> __('Color Palette','xe-core'),
			'instructions'	=> __('Enter color codes separated by semicolons. You can use HEX or RGBA color codes and can also mix them (e.g. #2ecc71; rgba(50,40,30,0.5).<br><br>This can (and is maybe) overwritten by the "acf/acfrb_color_picker/palette" filter.','xe-core'),
			'type'			=> 'text',
			'name'			=> 'color_palette'
		));
		...
		// hide palette
		acf_render_field_setting( $field, array(
			'label'			=> __('Hide Color Palette','xe-core'),
			'instructions'	=> __('Don\'t show a color palette in the color picker','xe-core'),
			'type'			=> 'true_false',
			'name'			=> 'hide_palette',
			'ui'			=> 1,
		));
